...
To combat this problem, various password management software systems have been introduced. Each of them makes a certain trade-off between convenience and security. We introduce \texttt{pshash} --- a
deterministic ``on-the-fly'' password generation algorithm based on
selecting characters from predefined sources. The selection of
characters and their order are derived from two secret numbers (the
``choice private key'' and ``shuffle private key'') provided by the
user, and the resulting character string can be used as a password.
Our algorithm differs from its alternatives in that it sacrifices a
great deal of convenience (the user is required to memorize a
relatively large amount of entropy to achieve good security), but at
this cost it can be extremely minimal, stateless, portable and self-sufficient, reducing the amount of attack surfaces. Implementations of our algorithm also provide methods to more easily remember the private keys, namely arithmetic expressions and mnemonic incantations. The main implementation is written in the Haskell language which fits perfectly for this purpose, practically making the program bug-free by means of strong typing and immutability.

\textbf{Keywords:}
Passwords, Authentication, Combinatorics, Set Functions, Injectivity, Functional Programming
...
\begin{itemize}
  ...
  \item \textbf{Using a deterministic ``on-the-fly'' password generator.} These are programs that allow the user to derive a password from their identity and a secret master password, using a stateless (usually hash-based) function. An example is PwdHash \cite{ross} as well as \cite{jubur}. This approach eliminates the problem of storage, since nothing is ever stored on disk. However, being based on well-known hash functions opens such schemes to dictionary attacks and brute-force attacks, although mitigated in \cite{conv}. In addition, deterministic password generators are still more complicated than they could be, and they often require users to provide their identity, a requirement that should not be necessary.
\end{itemize}

In this paper, we describe a new ``on-the-fly'' password generation algorithm \texttt{pshash} that differs from its alternatives in two important ways:

\begin{itemize}
  \item \texttt{pshash} is extremely minimalistic (written in under 200 lines of code) and standalone, which makes it very portable and easy to implement on any platform and in any programming language that supports arbitrary-precision arithmetic (see Section \ref{impl} for a discussion of \texttt{pshash}'s implementation). The program is detached from any external service or browser.

  \item Our algorithm uses a custom universal hash function instead of the ``industry standards'' of cryptographic hashing such as SHA-256 or Salsa20. This means that our algorithm is immune to attacks targeting these hash functions, such as dictionary attacks which use pre-computed hash tables.
\end{itemize}

The above benefits come at the cost of convenience, which means that \texttt{pshash} is probably ill-suited for mass adoption. However, it fits perfectly for technically educated users who do not mind memorizing a couple of 20+ digit numbers.

One final remark: our algorithm is by no means the ``best'' from a security or efficiency standpoint. It is in fact quite inefficient. We argue, however, that it is secure \emph{enough} and fast \emph{enough} for the purposes of personal password management, and more than anything, it serves as a proof of concept that can later be improved upon.
...
\begin{itemize}
  \item The name (or any other distinct identifier, for that matter) of the website for which a password is required, called the \emph{public key} and denoted $p$;
...
The function returns a string of characters which we call a ``pseudo-hash'' since it is not a full cryptographic hash but has some of its properties. \texttt{pshash} does not produce characters from the keys directly, but instead uses them to select characters from predefined strings, which makes it easier to naturally ensure that the pseudo-hash complies with a website's password requirements. The algorithm also supports different password templates, called \emph{source configurations.} We will now consider the algorithm in detail.
...
Let $n,m \in \N,\ m \leq n$. The quantity $n!/(n-m)! = n \cdot (n-1) \cdot \ldots \cdot (n-m+1)$ will be called a \emph{relative factorial} and denoted by $\rf{n}{m}$. Note that $\rf{n}{1} = n$ and $\rf{n}{n} = n!$.
...
The purpose of the public key $p$ is solely to allow the user to generate different passwords with the same set of private keys. Therefore, it need not be an integral part of the core \texttt{pshash} algorithm. In this and the following subsections we will build the function $\h(\conf, k_1, k_2)$ which accepts a source configuration and two private keys to produce a pseudo-hash, and the public key will be introduced later.\\
...
\begin{definition}
  ...
  \]
  This number exists due to $A$ being finite. We will denote the spread of $f$ by $\spr{f}$.\\
\end{definition}
...
\begin{proposition}
  ...
  \]
  where $T \colon \N_0 \to \N_0$ is a fixed function, referred to as an \textbf{argument shift function.} It is then stated that $\spr{h} \geq n m$.
\end{proposition}
\begin{proof}
  Assume that $k_1 \ne k_2$ and $h(k_1) = h(k_2)$. Since $h$ returns an ordered list, the equality of lists is equivalent to the equality of both their respective entries:
  \begin{align*}
...
\begin{definition}
  ...
  \]
  where $E' = E \wo \li^n k$ and $k' = \li_n k + T(\li^n k)$, while $T \colon \N_0 \to \N_0$ is some fixed argument shift function.\\
\end{definition}
...
The choice function gives us a way to enumerate all possible ways to select a sub-list from $E$. What is more, these selections can be made more ``random'' and unpredictable by means of complicating the argument shift function $T$. For example, we may set $T(\li^n k)$ to the ASCII value of the character $E : \li^n k$. This way, each chosen character will influence the choice of the next, making the function non-linear and unpredictable, preventing it from following any algebraic laws. Here is a little input-output table for the choice function of order 10 with the specified source and shift function:
...
\begin{definition}
  ...
  \]
  where $T \colon \N_0 \to \N_0$ is an argument shift function. The base of the recursion is given when $\conf$ is empty, in which case $\elv([], k) = []$. Otherwise, for every key $k$, its image under $\elv(\conf, -)$ is an element of
  \[
...
This solves the problem of lacking symbol categories --- now we can separate upper-case letters, lower-case letters, numbers, etc., into different sources and apply the elevated choice function, specifying the number of symbols from each source. However, there are two issues arising:
...
\begin{definition}
  Let $f \colon A \times B \to C$ be a function. We say that $f$ is
  \emph{absolutely injective} with respect to the first argument, if
  for all $(a_1, b_1), (a_2, b_2) \in A \times B$, the equality
  $f(a_1, b_1) = f(a_2, b_2)$ implies $a_1 = a_2$ (while $b_1$ and
  $b_2$ may differ). Similarly we can consider absolute injectivity
  with respect to the second argument.
\end{definition}

Sometimes this property is called just ``injectivity in the first
argument'', but we find it slightly confusing, since intuitively it
seems to mean that $f(-, b)$ is injective for any $b \in B$ (similarly
to $f$ being ``linear in the first argument''). Our property is,
however, stronger than that.\\

\begin{proposition}
  \label{comp}
  Let $f \colon A \times \N_0 \to B$ and $g \colon B \times \N_0 \to
  C$ be functions such that $\spr{f} \ge n$ and $\spr{g} \ge m$, where
  the spread is taken with respect to the second argument. Assume also
  that $g$ is absolutely injective with respect to the first argument.
  % \[
  ...
  \]
  where $T \colon \N_0 \to \N_0$ is an argument shift function. It is then stated that $\spr{h} \ge n m$ with respect to $k$.
\end{proposition}
\begin{proof}
...
\begin{definition}
  ...
  \begin{enumerate}
    ...
    \item Neither $\alpha$ nor $\beta$ is empty. Then we will assume that the merge function is already defined for $(\alpha \wo 0, \beta, -)$ and $(\alpha, \beta \wo 0, -)$. Let $s_1$ be the spread of the function $\m^2(\alpha \wo 0, \beta, -)$ and $s_2$ be the spread of $\m^2 (\alpha, \beta \wo 0, -)$. Finally, denote the remainder $\li^{(s_1+s_2)}k$ by $k'$. The merge of $\alpha$ and $\beta$ with key $k$ and an argument shift function $T \colon \alpha \uplus \beta \to \N_0$ is defined as
      \[
        \m^2(\alpha, \beta, k) = \begin{cases}
          [\alpha : 0] \uplus \m^2(\alpha \wo 0, \beta, k' + T(\alpha : 0)),\ \mathrm{if}\ k' < s_1,\\
          [\beta : 0] \uplus \m^2(\alpha, \beta \wo 0, k' + T(\beta : 0)),\ \mathrm{otherwise}.\\
        \end{cases}
      \]
  \end{enumerate}
\end{definition}
The merge function takes two lists and combines them together in one, in such a way that the order of elements in each of the two lists is not disturbed. For example, the merge of $[1,2,3]$ and $[a,b,c]$ with a certain key could be $[1,a,b,2,c,3]$. We will now derive some properties of $\m^2$. If $s_1$ and $s_2$ are as defined above, we immediately see that $\m^2(\alpha, \beta, -)$ is periodic with period $s_1 + s_2$, since it depends only on $k' = \li^{(s_1 + s_2)}k$. Hence, $\spr{\m^2 (\alpha, \beta, -)} \leq s_1 + s_2$. Moreover, we have the following result:\\

\begin{proposition}
  \label{m2inj}
  If $\alpha$ and $\beta$ are non-empty lists, the function $\m^2(\alpha, \beta, -)$ is injective on the interval $(s_1 + s_2)$, where $s_1, s_2$ are as given in Definition \ref{m2def}.
\end{proposition}
\begin{proof}
  Consider two numbers $k_1, k_2 \in (s_1 + s_2)$ such that
  \[
    \m^2(\alpha, \beta, k_1) = \m^2(\alpha, \beta, k_2) =: \gamma.
  \]
  Since the lists $\alpha$ and $\beta$ have empty intersection (being taken from distinct sources $E_1$ and $E_2$), we see that either $k_1, k_2 < s_1$ or $k_1, k_2 \ge s_1$, as otherwise we would have $\alpha : 0 = \gamma : 0 = \beta : 0$. If $k_1, k_2 < s_1$, we see from Definition \ref{m2def} that
  \[
    \m^2 (\alpha \wo 0, \beta, k_1 + T(\alpha : 0)) = \m^2 (\alpha \wo 0, \beta, k_2 + T(\alpha : 0)).
  \]
  Since $|k_1 - k_2| < s_1 = \spr{\m^2(\alpha \wo 0, \beta)}$, we must have $k_1 = k_2$ by the definition of spread. The case $k_1, k_2 \geq s_1$ is similar.
\end{proof}

The above proposition entails that the spread of $\m^2(\alpha, \beta, -)$ equals exactly $s_1 + s_2$, since, being periodic, this function will be injective on any interval of length $s_1 + s_2$. Therefore, we have the identity
\begin{align}
  \label{recspr}
...
% \begin{proposition}
%   \label{m2surj}
%   For any lists $\alpha \in [E_1]_{m_1}$ and $\beta \in [E_2]_{m_2}$, the function $\m^2(\alpha, \beta, -)$ is surjective.
% \end{proposition}
% \begin{proof}
%   We again employ induction over $m_1 + m_2$, with the base case being where either $m_1 = 0$ or $m_2 = 0$
% \end{proof}
%
...
Now we will expand the merge function to an arbitrary number of lists.\\
...
\begin{definition}
  ...
  \begin{align}
    ...
    \m^N (\overline{\alpha}, k) = \begin{cases}
      ...
      \m^2 (\alpha_0, \m^{N-1} (\overline{\alpha} \wo 0, \li^s k), \li_s k + T(\alpha_0)),\ \ N > 2,
    \end{cases}
  \end{align}
  where $s$ is the spread of the function $\m^{N-1} (\overline{\alpha} \wo 0, -)$, and $T \colon \overline{\alpha} \to \N_0$ is an argument shift function.\\
\end{definition}

\begin{proposition}
  Let $\overline{\alpha} = [\alpha_0, \alpha_1, ..., \alpha_N]$ be a list of lists with $N \ge 2$ and $\alpha_i \in [E_i]_{m_i}$. Then the function $\m^N (\overline{\alpha}, -)$ is periodic with the period of its spread, and we have
  \begin{align}
    \label{sprmN}
...
\end{proposition}
\begin{proof}
  We conduct a proof by induction over $N$. If $N = 2$, the result follows from Proposition \ref{sprm2}. Now, assume that the result holds for all merge functions of order $N-1$. Note that (\ref{mNdef}) follows the definition scheme assumed in Proposition \ref{comp}. Moreover, observe the recursive relation
  \[
    \frac{\left( \sum_{i = 0}^{N-1} m_i \right)!}{\prod_{i = 0}^{N-1} m_i !} = \frac{\left( m_0 + \sum_{i=1}^{N-1} m_i \right )!}{m_0! \cdot \left( \sum_{i=1}^{N-1} m_i \right)! } \cdot \frac{\left( \sum_{i = 1}^{N-1} m_i \right)!}{\prod_{i = 1}^{N-1} m_i !} = \spr{\m^2 (\alpha_0, \m^{N-1}(\overline{\alpha} \wo 0, x), -)} \cdot \spr{\m^{N-1}(\overline{\alpha} \wo 0, -)},
  \]
  where $x \in \N_0$ is any fixed number. Then, applying Proposition \ref{comp}, we see that the $\ge$ sign in (\ref{sprmN}) holds for $\m^N (\overline{\alpha}, -)$.\\
  Now, denoting $m = \spr{\m^2 (\alpha_0, \m^{N-1}(\overline{\alpha} \wo 0, x), -)}$ and $n = \spr{\m^{N-1}(\overline{\alpha} \wo 0, -)}$, for all $k \in \N_0$ we have
  \begin{align*}
    \m^N(\overline{\alpha}, k + mn) &= \m^2(\alpha_0, \m^{N-1}(\overline{\alpha} \wo 0, \li^n (k + mn)), \li_n (k + mn) + T(\alpha_0))\\
    &= \m^2(\alpha_0, \m^{N-1}(\overline{\alpha} \wo 0, \li^n k), m + \li_n k + T(\alpha_0)) = \m^N (\overline{\alpha}, k),
  \end{align*}
  so the function $\m^N(\overline{\alpha}, -)$ is periodic with period $mn$, which means that its spread is equal to $mn$, as desired.
\end{proof}
...
\begin{definition}
  ...
  \]
  where $n$ is the spread of the elevated choice function $\elv(\conf, -)$, while $T \colon \N_0 \to \N_0$ is a fixed argument shift function.\\
\end{definition}
...
\begin{proposition}
  Let $\conf$ be a source configuration of length $N$. The following equality takes place for the spread of the merged choice function:
  \begin{align}
    \label{sprmc}
    \spr{\mc(\conf, -)} = \spr{\elv(\conf, -)} \cdot \spr{\m^N (\overline{\alpha}, -)} = \prod_{i = 0}^{N-1} \rf{n_i}{m_i} \cdot \frac{\left( \sum_{i = 0}^{N-1} m_i \right)!}{\prod_{i = 0}^{N-1} m_i !},
  \end{align}
  where $\overline{\alpha}$ is the multiselection arising from the application of $\elv$, while $n_i$ are the lengths of the sources $E_i$ in the configuration $\conf$. Moreover, the function $\mc(\conf, -)$ is periodic with the period of its spread.
\end{proposition}
\begin{proof}
  Since the merge function $\m^N$ preserves the order of the lists it merges, we can see that it is absolutely injective with respect to its first argument, $\overline{\alpha}$. Indeed, no matter how the multiselection $\overline{\alpha}$ is merged, all elements of, say, $E_0$, can be read from the resulting merged list in the order that they were originally. In other words, the list $\alpha_0 \in \overline{\alpha}$ can be reconstructed from the output of $\m^N (\overline{\alpha}, k)$ for all $k$. And so can $\alpha_1$, $\alpha_2$, etc. Here we are actively using the fact that all elements across all sources $E_0, E_1, ..., E_{N-1}$ are distinct. Now that the absolute injectivity of $M^N$ has been established, the $\ge$ sign in (\ref{sprmc}) immediately follows from an application of Proposition \ref{comp}. 
  Now, it is easy to check by definition and using the periodicity of $\m^N(\overline{\alpha}, -)$, that for all $k \in \N_0$, we have
  \[
    \mc(\conf, k) = \mc(\conf, k + m n),
  \]
  where $n = \spr{\elv(\conf, -)},\ m = \spr{\m^N (\overline{\alpha}, -)}$. This means that $\mc(\conf, -)$ is periodic with period $m n$, and the inequality in (\ref{sprmc}) is in fact an equality.
\end{proof}
...
\subsection{Subverting Invertibility} \label{subv}

We still have one problem left: our current pseudo-hash function is bijective on its spread, and it can be inverted relatively easily to retrieve the private key from the source configuration and the final pseudo-hash. To prevent this, we will have to make our function a bit less injective --- artificially add inputs that produce the same output, in order to make the function harder to invert.\\
...
We will also assume that an adversary can perform $10^{10}$ password checks per second, i.e. 0.1 nanosecond to perform one operation such as computing a pseudo-hash or computing a key pair that produces a given pseudo-hash value. This measure is in fact extremely conservative. For example, running \verb|openssl speed -evp sha256| to benchmark the speed of SHA-256 on a 22-core Intel Ultra 9 185H CPU has yielded a speed of $10,000,000$ hashes per second (performed on 16-byte blocks), or 100 nanoseconds per hash. We assume here that \texttt{pshash} is $1,000$ times faster, whereas in fact it must be orders of magnitude slower, due to the fact that it must perform many multiplications and divisions on arbitrary-precision integers. This conservative speed estimate serves as a guarantee of security for a long period of time, even when processors become a million times faster than they are today.
...
Being a standalone original algorithm, \texttt{pshash} is very resistant to supply chain attacks. However, this also means \texttt{pshash} does not protect from phishing and keyloggers, being completely unaware of them. Hence, \texttt{pshash} should be used with care, as it is only responsible for producing a password, and not for how it is used afterwards.
...
The general invocation of \texttt{pshash} is \verb|$ pshash PUBLIC CHOICE SHUFFLE|, providing the public key and the choice-shuffle private key-pair. To avoid passing secrets directly as command-line arguments, the user may omit any of the three arguments and provide them via \texttt{stdin}. In addition, the program has many flags and options, all of which can be viewed by invoking \verb|$ pshash --help|. We will expand on some of the most important features below. Note that some of the details may differ depending on the version of \texttt{pshash}. Here we consider version \texttt{0.1.23.0}.

\subsection{Simplified Key Input and Generation} \label{input}

One of the biggest disadvantages of \texttt{pshash} is the size of the private keys. For the source configuration given in Section \ref{security}, the maximum relevant choice key is on the order of $2^{142}$ and the maximum shuffle key on the order of $2^{83}$. Such long numbers are possible to remember using mnemonics, but they still take a lot of effort to commit to memory, and the cost of forgetting even a single digit is the permanent loss of all passwords derived from this key-pair. Additionally, even coming up with such big numbers is hard for a human to do. One would inevitably think of numbers relevant in their life, which makes them vulnerable to attacks through social analysis.
...
\begin{itemize}
  ...
  \item The program supports alternative input methods for the keys.
    For example, it can evaluate arithmetic expressions involving
    addition, multiplication, and exponentiation. Instead of
    \texttt{52046849220153179918678225216} one may type \texttt{8234 *
    91234 \^{} 5} which is much easier to remember. Of course, if this
    input method becomes mainstream, a brute-force attack on such
    passwords will be much easier, since it will be enough to check
    all numbers of the form $a + b$, $a * b$, $a ^ b$ and some basic
    combinations of these expressions, where $a,b < 100,000$. Hence
    the arithmetic input method is not recommended.\\
    As a safer alternative, the \texttt{pshash} implementation offers a ``mnemonic incantation system''. By selecting the 100 most used English 2-letter syllables, we associate each 2-digit number with a syllable. For example, \texttt{00} corresponds to \texttt{'or'}, \texttt{37} corresponds to \texttt{'mi'}, and \texttt{99} corresponds to \texttt{'je'}. We can then apply this map to whole numbers, so \texttt{993700} becomes \texttt{jemior}, which sounds like an English word and is much easier to remember. Given the source configuration described in Section \ref{security}, the average choice key will have about 7 three-syllable words in its mnemonic incantation, while the average shuffle key will have about 4 words. An example would be
    \begin{align*}
    ...
    \end{align*}
    Remembering these incantations still requires effort, of course,
    but it is feasible (and gets progressively easier with practice), and the nature of \texttt{pshash} is such that the security properties of passwords rely directly on the amount of entropy in the keys.\\
...
Apart from remembering the keys, the user will have to input these keys quite frequently, since they are never stored physically. It can become very inconvenient to precisely type about 70 characters each time one needs to generate a password. To combat this problem, the primary \texttt{pshash} implementation offers the \verb|--loop| and \verb|--clip| flags. Executing \verb|$ pshash --loop| will prompt the user for the two \textit{private} keys and bring them to an interactive prompt, where different public keys can be supplied. On demand, \texttt{pshash} will take a public key and produce a pseudo-hash using it and the two pre-supplied private keys. This way, the user only need to type in the private keys once. Additionally, the user may pass \verb|--clip| (together with \verb|--loop|, or inside the interactive prompt) to immediately copy the resulting pseudo-hashes to the system clipboard. This flag relies on external programs (\texttt{clip.exe} on Windows, \texttt{xclip}/\texttt{wl-copy} on Linux), so the user must make sure to install them, otherwise \texttt{pshash} will return an error upon trying to pass the \verb|--clip| flag.
...
\subsection{Shortening Keys}%
The relevant length of the private keys depends on the source configuration. For the default configuration given in Section \ref{security}, the length of the choice key is about $2^{142}$, while the length of the shuffle key is about $2^{83}$. However, for smaller configurations the relevant key size is also much smaller. For example, the configuration which produces 6-digit pin codes only requires choice keys in the range from $0$ to $151200$ (the spread of the merge-choice function) and shuffle keys in range from $0$ to $720$ (the spread of the shuffle function). This is because the resulting passwords (6-digit number sequences) have very little entropy, so the keys also need not have much entropy.

Let $\conf$ be a source configuration, and let $\conf'$ be another, smaller configuration (for instance, $\conf$ may be as defined in Section \ref{security}, resulting in 25-letter pseudo-hashes, while $\conf'$ selects a 6-digit pin code, resulting only in 6-letter hashes). $(k_1, k_2)$ be a pair of private keys where $k_1 \in \left(\#^\mc (\conf)\right)$ and $k_2 \in \left(\#^\s (\conf)\right)$. From the periodicity of the function $\h(\conf', -, -)$ with respect to both arguments, we see that
\[
  \h(\conf', k_1, k_2) = \h(\conf', k'_1, k'_2),
\]
where $k'_1 = \li^{\#^\mc (\conf')} k_1$ and $k'_2 = \li^{\#^\s (\conf')}k_2$. The key-pair $(k'_1, k'_2)$ is thus equivalent to $(k_1, k_2)$ for $\conf'$, but may be much easier to remember. The primary \texttt{pshash} implementation includes the \verb|--gen-mod| operation, which prompts for two private keys and reduces them modulo the used source configuration. For example, \verb|$ pshash -k mediumpin --gen-mod| will reduce
\begin{align*}
  3945833357794975306816070866530177716272967 \ &\longrightarrow \ 14567\\
  9720291686847129543734580 \ &\longrightarrow \ 660
\end{align*}
And when generating 6-digit pin codes, the user may now simply supply $14567$ and $660$.

\subsection{File Encryption}

Recall that the $\s(-, k)$ function from Subsection \ref{subv} can shuffle any ordered list of distinct elements, given a key $k$. This allows us to transform the \texttt{pshash} algorithm into a simple block cipher (\cite{serious}, page 93). For a number of rounds $r$ and two private keys $k_1, k_2$, we define a permutation by
\[
  \varphi = (\s(-, k_2) \circ \s(-, k_1))^r,
\]
Applying this permutation to the list $[0, 1, 2, ..., 63]$, we receive a permuted list $[n_0, n_1, ..., n_{63}]$. Now, given a plaintext 64-byte block $P = [P_0, P_1, ..., P_{63}]$, we obtain the corresponding ciphertext block by shuffling its bytes according to the permutation:
\[
  C = e_\varphi (P) = [P_{n_0}, P_{n_1}, ..., P_{n_{63}}].
\]
To decrypt $C$, it suffices to invert the permutation $\varphi$ and repeat the same procedure, i.e. $P = e_{\varphi^{-1}} (C)$.

As before, the security of this cipher does not correspond to the conventional notion. Given a ciphertext block $C$, one can easily generate a pair of keys and a plaintext block that will produce $C$ under the above procedure (for example, one can take $k_1 = k_2 = 0$ and calculate the corresponding plaintext). However, there is no information in $C$ about the \emph{particular} key-pair and plaintext that were used, and the amount of possible options is equal to
\[
  \spr{\s([0,...,63], -)} = 64! \approx 2^{295},
\]
which is simply the number of all permutations on $64$ elements.

Now, to encrypt/decrypt byte streams of length other than 64, we run our block cipher in (a slightly modified) counter mode (\cite{serious}, page 118). In particular, let a plaintext $P$ of arbitrary size be given, along with two keys $k_1, k_2$. We calculate the permutation $\varphi$ based on $k_1, k_2$, and we also generate a random 64-byte block $IV$. Now, we build a sequence $(B_i)_{i = 0}^\infty$ of 64-byte blocks as follows:
\begin{itemize}
  \item We set $B_0$ to $e_\varphi (IV \oplus DS)$, where $DS$ is a fixed random 64-byte block generated beforehand and hard-coded in the executable;
  \item If $B_i$ is computed, we set $B_{i+1} = e_\varphi (B_i \oplus DS)$.
\end{itemize}
Now, to encrypt $P$, we simply concatenate $(B_i)_{i = 0}^\infty$ into a stream of bytes $B$ and then XOR it with $P$, prepending the initial value $IV$ for decryption:
\[
...
This simple cipher is included in \texttt{pshash}'s primary
implementation. By invoking \verb|$ pshash -e FILE|, the user will be
prompted for the destination \verb|OUT|, as well as two keys, after
which the contents of \verb|FILE| will be encrypted and written to \verb|OUT|. The program can read from \texttt{stdin} and write to \texttt{stdout} if \verb|FILE| is \verb|"stdin"| or \verb|OUT| is \verb|"stdout"|. Of course, both keys can be specified as arithmetic expressions or mnemonic incantations.

\subsection{Other implementations}

The \texttt{pshash} algorithm is implemented (at least in its skeleton form) in other languages and deployed on other platforms:

\begin{itemize}
  \item In \textbf{C}, using the GNU Multiple Precision Arithmetic library \cite{gmp}. This serves as a faster bare-bones implementation of \texttt{pshash} (since Haskell is often very slow), and also facilitates the in-development \texttt{pshash} desktop app \cite{pshash-gui-src};

  \item In \textbf{JavaScript}, to facilitate the \texttt{pshash} web interface \cite{pshash}. This is a very important deployment location since it allows users to generate passwords from any machine that has internet access, without installing anything locally. The web interface is a static HTML page hosted via GitHub Pages, with no server-side interaction. The passwords are generated by JavaScript on the client side and can then be either displayed or copied to the clipboard;

  \item In \textbf{Kotlin} for the \texttt{pshash} Android application \cite{pshash-app-src}.
\end{itemize}

All these implementations support the additional key input methods described in Subsection \ref{input}.

\section{Conclusion}

We have presented the \texttt{pshash} password generation algorithm that differs from its closest analogues by its simplicity and originality. \texttt{pshash} provides little user convenience, since the secrets that need to be remembered by the user are big in size. This problem is partly mitigated in the implementation of \texttt{pshash} with two alternative key input methods. In exchange the algorithm is completely stateless, as well as very portable and minimalistic. The implementation has many other features as well, but it provides no integration with the browser or any external program, for the sake of independence, but at the cost of convenience. Finally, other implementations of \texttt{pshash} are available, which makes the algorithm accessible from any platform and on third-party machines.
