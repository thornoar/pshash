...
Now, it is easy to check by definition that
\[
  \mc(\conf, 0) = \mc(\conf, \spr{\elv(\conf, -)} \cdot \spr{\m^N (\overline{\alpha}, -)}),
\]
which means that the inequality in (\ref{sprmc}) is in fact an equality.
