...
\begin{itemize}
  \item The domain name (or any other distinct identifier, for that matter) of the website for which a password is required, called the \emph{public key} and denoted $p$;
...
The choice function gives us a way to enumerate all possible ways to select a sub-list from $E$. What is more, these selections can be made more ``random'' and unpredictable by means of complicating the argument shift function $T$. For example, we may set $T(\li^n k)$ to the ASCII value of the character $E : \li^n k$. This way, each chosen character will influence the choice of the next, creating somewhat of a ``chaotic transformation'', where its behavior is fully determined, but even small changes to inputs produce large changes in the output. Here is a little input-output table for the choice function of order 10 with the specified source and shift function:
...
We have presented the \texttt{pshash} password generation algorithm that differs from its closest analogues by its simplicity and originality. The minimalistic design of \texttt{pshash} comes at the cost of user convenience, since the secrets that need to be remembered by the user are big in size. This problem is partly mitigated in the implementation of \texttt{pshash} with two alternative key input methods. The implementation has many other features as well, but it provides no integration with the browser or any external program, for the sake of independence, but at the cost of convenience. Finally, other implementations of \texttt{pshash} are available, which makes the algorithm accessible from any platform and on third-party machines.
