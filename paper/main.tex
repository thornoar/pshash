...
\begin{itemize}
  ...
  \item \textbf{Using a password manager.} \cite{passman} These are software programs that create random passwords for the user and give access to them via a \emph{master password.} This is a valid solution, especially since password managers tend to employ very secure encryption algorithms. However, this using such software implies problematic notions such as
    \begin{enumerate}
...
\texttt{pshash} (from ``pseudo-hash'') is the name of the implementation of the algorithm specified in the previous section. The main program has no GUI, it is only a command-line utility. This decision was made for the sake of simplicity, speed, and portability. The Haskell source is available at \url{https://github.com/thornoar/pshash}, together with installation instructions and other details.
...
\begin{itemize}
  ...
  \item \textbf{JavaScript} for the web interface, available at \url{https://thornoar.github.io/pshash-web/app/};
