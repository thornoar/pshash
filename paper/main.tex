...
\begin{itemize}
  ...
  \item In \textbf{JavaScript}, to facilitate the \texttt{pshash} web interface \cite{pshash}. This is a very important deployment since it allows users to generate passwords from any machine that has internet access, without installing anything locally. The web interface is a static HTML page hosted via GitHub Pages, with no server-side interaction. The passwords are generated by JavaScript on the page and can then be either displayed or copied to the clipboard;
