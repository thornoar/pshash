#!/usr/bin/env bash

cmd="$1"

test_number=1

function compare {
    printf "Test \033[34m#%02d: \033[0m%-55.55s\033[34m -- \033[0m" "$test_number" "$1"
    output="$($1)"
    if [[ "$output" == "$2" ]]; then
        printf "\033[1;32mPASSED\033[0m\n"
        test_number=$((test_number+1))
    else
        printf "\033[1;31mFAILED\033[0m\n"
        printf "    Wanted: \033[33m%s\033[0m\n" "$2"
        printf "    Got:    \033[33m%s\033[0m\n" "$output"
        exit 1
    fi
}

compare "$cmd --version" "The pshash pseudo-hash password manager, version 0.1.22.0"
compare "$cmd zxc-%-vbn 89^45 9045^4557" "FkPw=p+VyMjdv6XN\$2^3i@QW4"
compare "$cmd -k long AAA0-=~!@ 123 123" "@&FjQ*34kzZu1O\$IGncBb%5Hy"
compare "$cmd -k medium ##### 9^900*30*99 125^125" "5y=D06Zw*K\$N%8k@1raX"
compare "$cmd -k short ##### 123^1000 666^1" "H\$!x9*PMZ#z6f4h5"
compare "$cmd -k anlong --pure ##### 123^1000 666^1" "bWZHh3y92TzV84XBx6af0"
compare "$cmd -k max --pure gtgt4gt4 123*123432+500^3^2 666" "cB#C56-^Dek?t8=sl+9waWOLQX0pM\$Zy1jJ3F7Ib%fx4!qodiKNR2HY@Sh&VrgEnPu*TAvzGmU"
compare "$cmd -k anshort ##### 123^1000 666^1" "3xPM47Zz2Hfh"
compare "$cmd -k pin ##### 12*43^10+120 666^1" "6720"
compare "$cmd -k mediumpin qwerty 89 9045" "185074"
compare "$cmd -k longpin tyuio 1000 9000" "30624859"
compare "$cmd -n (4,6,5,5) hahaha 456 123" "&Uo0b\$u1*=FdEVW4O+26"
compare "$cmd -n (26,26,0,0) public 456^345 123^0" "RHdyqxtYoAfnwCPXjGTeprmzbcghFJZMuINsaiSElLvOUBkVDQWK"
compare "$cmd -c [(\"^&*#\",3),(\"cfrvgtbhy\",8),(\"AZSXDCFV\",5)] e3r4t5 5000 1000" "cZD^Ffgtrvh&XbA#"
compare "$cmd -q public --plain 735^5 537^5 X+I?3tioB4f\$y=P^6CkN2SYjp" "\"vftyhb135\""
compare "$cmd -q choice --plain vftyhb135 537^5 X+I?3tioB4f\$y=P^6CkN2SYjp" "214504642209375"
compare "$cmd -q shuffle --plain vftyhb135 735^5 X+I?3tioB4f\$y=P^6CkN2SYjp" "44655137246457"
compare "$cmd --list --plain 1asdfgh1 1 dU*wG6zh!^I0CfqB3&MF5jt-W" "4671573659263123921383069281900060449000 0"
compare "$cmd -f test/pshash.conf google 123 123" "03NbhL7aoSVGRYjt28y19"
compare "$cmd --impure -f test/pshash.conf -q choice phone 123 123456 --plain" "79601"
compare "$cmd --pure --impure -f test/non-existent asd345345 mufasa 45*34^3" "zaYwX4IKQCmoM=?2\$3k6xJ^+y"
compare "$cmd -k anlong -p 66 qwer 90 90" "Lh5b1q92I4VzRriNB87eU"
compare "$cmd -f test/pshash.conf overleaf 876^5 139 -p 4" "3F1SVZD"
$cmd -r 24 -e "test/data.txt" "test/data.enc" "3498*34^3" "pijekoduriloyepaorlemesu"
compare "$cmd -r 24 -d test/data.enc stdout 3498*34^3 pijekoduriloyepaorlemesu" "$(cat "test/data.txt")"
compare "$cmd -k anlong puuub uskehaunpicadopebewokabeitagelelcavemuduunba tifijalierfumeterotejoquve" "K4QEmk0hx5uYB3vFb187S"
compare "$cmd -n (24,13,10,7) magiiiic 1234^247+1237*123 ityilereorbafafapijahofemefule" "ecdwLxq@^IayFC2SEb%-nr8Blvz*6#O1mZAhg!5o?ut+psj9\$QiW7X"
compare "$cmd -i numbers --plain -n (3,4,4,5)" "$(printf "101447155469153894400000000\n20922789888000\n20922789888000")"
compare "$cmd -i times --plain -n (24,3,0,0)" "$(printf "14578254819129952\n17252372567")"
compare "$cmd --gen-spell --plain 123^11" "yudayi tikipu higogi suwaca"
compare "$cmd --gen-num --plain uspajacasidesemerurutoxegiorfigesigunubuvaco" "5469727312221326363389662005542318486737623"
compare "$cmd --gen-mod --plain ___ hahamuhahaha kiwami" "$(printf "444480444444\n697837")"
compare "$cmd --gen-mod --plain -f test/pshash.conf google 2344^34+6 mahasukimaru" "$(printf "1909240440344555461659735460151302\n364457693663")"
